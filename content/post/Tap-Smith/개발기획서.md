...
- **핵심 목표**: 실패 시 하락 없는 '직진형(No Back)' 강화 시스템에서, 파괴의 공포를 이겨내고 최고의 무기를 만들어 비싸게 파는 것임.
...
## 3. 게임 루프 (Core Loop)
MVP 버전에서는 전투 콘텐츠를 제외하고 **경제 순환**에 집중할 계획임.
...
각 강화 단계의 가치는 **"파괴 시 0강부터 다시 복구해야 하는 리스크 비용"**을 포함하여 계산할 예정임.

$$EV_{next} = EV_{current} + \frac{Cost + (P_{break} \times EV_{current})}{P_{success}}$$

이 공식을 기반으로 판매 가격은 `EV * (0.6 ~ 0.8)` 수준으로 책정하여, 유저가 **"직접 띄울까, 그냥 살까"**를 고민하게 만드는 게 포인트임.
...
#### **Phase 3: 하드코어 (+20 ~ +29)**
**특징**: 파괴 확률 > 성공 확률. 무기 삭제 확률이 더 높음. 가격이 기하급수적으로 폭등.
...
강화 단계에 따라 무기의 **이름**과 **프로필 이펙트**가 변경됨.
...
## 6. 결론 및 향후 계획
현재 핵심 로직 밸런싱 작업 중임. 
단순한 운빨 게임을 넘어, 유저가 리스크를 관리하는 재미를 느낄 수 있도록 다듬는 게 목표임. 
조만간 실제 테스트 영상도 한 번 올려보겠음! 💪
