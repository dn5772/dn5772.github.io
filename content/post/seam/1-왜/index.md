...
description: "Cursor에서 하던 작업을 Claude Desktop이 모른다. 그 사이에 뭘 하나 끼워넣고 싶었음"
slug: "seam-1"
...
AI 도구를 여러 개 쓰다 보면 계속 걸리는 게 있다.

Cursor에서 인증 API를 만들다가 Claude Desktop을 열면, **맥락을 처음부터 다시 설명해야 한다.**
어제 AI랑 한 작업을 오늘 AI는 모른다. 같은 컴퓨터, 같은 사람인데.

이걸 해결하는 걸 만들어보기로 했다. 오늘은 코드 한 줄도 안 쓰고 설계 문서만 10개 썼다.
...
**seamless** 에서 따온 것도 있다. 사용자가 이음새의 존재를 못 느끼는 게 목표라서.

처음엔 "AI OS"라는 이름으로 기획했었는데(v0.1~v0.3), OS라고 하니까 너무 거창하고 실제로 하는 일이랑도 안 맞았다.
커널이나 스케줄러를 만드는 게 아니라 **그냥 중간에 끼는 레이어**이다.
...
40개 깔아도 대화 시작할 때 토큰을 안 쓴다. 요청 보고 필요한 것만 그 순간에 로드.
...
**Layer 0이 제일 중요하다.** 기존 앱들이 seam을 거치게 만드는 게 여기서 결정된다.
...
환경변수만 바꾸면 **앱은 자기가 프록시를 거치는지도 모른다.**

## 2티어 — Tier 1은 미끼, Tier 2가 본체

이게 채택 전략의 핵심이다.

**Tier 1 (프록시)**: 쓰던 앱 그대로 쓴다. seam이 사이에 껴서 메모리 주입하고 기록한다.
가입 저항이 0이다. 설치하고 환경변수만 바뀌면 끝.

**Tier 2 (네이티브)**: `seam "요청"` 으로 직접 쓴다. 전체 파이프라인을 다 쓴다.
...
모든 AI 요청이 seam을 거치게 만들면, **seam이 단일 장애점**이 된다.
게이트웨이가 죽으면 Cursor도 못 쓰고 Claude Code도 못 쓴다. 그건 최악이다.

그래서 passthrough 폴백을 설계에 박아뒀다.
...
사용자한테 보장하는 건 한 줄이다. **"seam이 죽어도 AI 앱은 동작한다."**

## 열린 질문을 같이 적었음

모르는 걸 모른다고 적어두는 게 나중에 도움이 될 것 같았다.
...
하루 종일 문서만 10개 썼는데 코드보다 이게 더 힘들었다.

제일 오래 붙든 건 **"이걸 왜 쓰지?"** 였다.
기능을 나열하는 건 쉬운데, 사용자가 설치할 이유 하나를 만드는 게 어려웠다. Tier 1을 미끼로 두는 구조가 그 답이었다.

근데 이 설계가 일주일 뒤에 **통째로 갈아엎힌다.** 다음 글에서.
