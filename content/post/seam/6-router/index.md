...
[#1]({{< ref "/post/seam/1-왜" >}})의 핵심 아이디어 5개 중 두 번째가 **"LLM 추상화 — 사용자는 모델을 몰라도 된다"** 였음.

말은 좋은데 막상 구현하려니까 문제가 있음. **사용자가 모델을 모르면, 비싼 모델을 써도 모름.**
알아서 골라주는 게 좋다가도, 청구서를 보면 얘기가 달라짐.
...
"어떤 모델 쓸래?" 대신 **"돈이랑 품질 중에 어디에 가까울래?"** 를 물음.
모델 이름은 계속 몰라도 되는데, 비용 방향은 내가 쥐고 있음.

값은 `~/.seam/config.toml` 에 들어가고, 범위를 벗어나면 0~9로 잘라냄.

```
22:29  fix: add Clone to SeamAppConfig, clamp router_slider to 0-9 on load
```

설정 파일은 사람이 손으로 고칠 수 있으니까 `router_slider = 42` 가 들어올 수 있음. 읽을 때 캡핑함.

## 🧠 복잡도는 누가 판정하나

슬라이더만으론 부족함. "이 요청이 간단한가 복잡한가"를 알아야 3~8 구간에서 갈래를 탈 수 있음.

근데 **복잡도를 판정하려고 GPT-4를 부르면 본말전도**임. 판정 비용이 실행 비용보다 비쌀 수 있음.

그래서 **로컬 Ollama 소형 모델**한테 시킴.
...
Ollama가 안 떠 있거나 느리면 **휴리스틱으로 폴백**함. 텍스트 길이 기준.

```
22:20  feat: implement Ollama complexity classifier with heuristic fallback
```

이 폴백이 꼭 필요했던 게, **분류기가 전체 파이프라인의 단일 장애점이 되면 안 되기 때문**임.
Ollama를 안 깐 사람도 seam을 쓸 수 있어야 함. 라우팅 품질만 떨어지고 동작은 해야 함.

[#1]({{< ref "/post/seam/1-왜" >}})에서 "Gateway가 죽어도 앱은 돌아가야 한다"고 적었던 거랑 같은 원칙임.
...
라우팅 표가 (복잡도 × 슬라이더) → 프로바이더 + 모델로 가는데, **선택된 프로바이더에 키가 없으면** 다음으로 넘어감.
...
Google을 나중에 끼워넣었는데, `medium` 등급 모델이 비어 있어서 폴백 체인이 중간에 끊겼음.
**세 단계(simple/medium/complex)를 프로바이더마다 다 채워야** 표가 완성됨.
...
[#1]({{< ref "/post/seam/1-왜" >}})에서 "code editor에서 한 작업을 planner가 안다"고 했던 게 이 `shared` 레이어임.
그냥 다 공유하면 될 것 같은데 그러면 안 되는 게, **환경마다 관련 없는 맥락이 섞임.**

백엔드 환경에서 한 작업이 영상 편집 환경 프롬프트에 끼어들면 노이즈임. 토큰도 낭비고.
...
그래서 **저장할 때 어디에 넣을지를 고르게** 함. IPC에 핸들러를 따로 뒀음.
...
작은 건데, 요청 하나 처리하면서 **OS 키체인을 두 번 조회**하고 있었음.
macOS 키체인 접근은 생각보다 안 싸고, 프록시는 요청마다 도는 경로라서 그대로 지연이 됨.

## 🎯 느낀 점

**"알아서 해준다"에는 항상 통제 손잡이가 하나 필요한 것 같음.**

모델 선택을 완전히 자동화하면 편한데, 사용자가 통제감을 잃음.
모델 이름을 다 보여주면 통제는 되는데 [#1]({{< ref "/post/seam/1-왜" >}})의 문제로 되돌아감.

슬라이더 하나가 그 사이 절충이었음. **추상화는 유지하되, 사용자가 신경 쓰는 축(비용)만 노출.**

그리고 분류기를 로컬 모델로 돌린 게 좋았음. 판정이 공짜라서 **모든 요청에 다 걸 수 있음.**
API를 불렀으면 "복잡도 판정할 가치가 있는 요청인가"를 또 판단해야 했을 것.

#seam #라우팅 #Ollama #LLM #비용
