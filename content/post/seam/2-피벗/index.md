...
[#1]({{< ref "/post/seam/1-왜" >}})에서 6레이어 아키텍처랑 2티어 전략을 짜고, 다음 날 Phase 1 MVP 구현 계획(Sprint 1~7)까지 세웠음.

그리고 일주일 뒤에 그걸 전부 버렸음.
...
투명하게 끼어드는 레이어가 아니라, **환경을 정의하고 패키징해서 공유하는 런타임**으로 바꿨음.

## 🤔 왜 바꿨나

v0.4의 핵심 가치는 "설치만 하면 AI가 나를 기억함"이었음.
좋은데, **사용자가 그걸 말로 설명하기가 어려움.**

"백그라운드에서 돌면서 메모리를 주입해주는 프록시"를 누구한테 설명해보면, 듣는 사람 표정이 애매해짐.
안 보이는 게 핵심인 제품은 자랑하기가 어려움.
...
**"Docker처럼 AI 환경을 통째로 공유한다"** 는 한 문장으로 끝남.

## 📄 seam.yaml

Docker Compose의 `docker-compose.yaml` 에 해당하는 파일임. AI 환경 전체를 선언함.
...
이 구분이 [#1]({{< ref "/post/seam/1-왜" >}})의 2티어를 **yaml 안으로 끌고 들어온 것**임.

- **native**: seam 바이너리가 서브프로세스로 직접 실행함. 스킬·라우팅 전부 적용됨
- **external**: Cursor 같은 기존 앱. seam이 실행하진 않고, 환경변수 오버라이드로 Gateway에 연결만 함. 메모리 주입만 받음

Tier 1/Tier 2가 "사용자가 어느 단계에 있나"였는데, 이제는 **"한 환경 안에서 툴마다 다름"** 이 됐음.
Cursor랑 native 에이전트를 같은 `seam.yaml` 에 적고 같이 띄울 수 있음.
...
레지스트리는 **GitHub 레포를 그대로 씀.** `seam-envs` 라는 레포에 Contents API로 파일을 올리고 내림.
자체 서버를 안 만든 이유는 단순함 — 지금 단계에서 서버 운영은 과함. 인증도 GitHub 토큰 하나면 끝남.

## 🗑️ 버린 것들

피벗하면서 v0.4 설계의 상당 부분이 뒤로 밀렸음.
...
설계 문서는 안 지웠음. `docs/archive/` 에 `ai-os-v0_3.md`, `seam-v0_4.md` 로 넣어뒀음.
**나중에 돌아올 수도 있고, 왜 이렇게 됐는지 알려면 이전 버전이 있어야 함.**

## 🎯 느낀 점

일주일 만에 갈아엎는 게 좀 허탈하긴 했는데, **코드를 한 줄도 안 쓴 상태에서 갈아엎은 게 다행**이었음.
Sprint 1~7을 반쯤 구현하고 나서 알았으면 훨씬 아팠을 것.

설계 문서를 먼저 쓰는 게 느려 보이는데, **버릴 때 싸게 버릴 수 있음.**

그리고 "안 보이는 게 핵심인 제품"은 만들기도 어렵고 설명하기도 어렵다는 걸 배웠음.
같은 기능이라도 `seam.yaml` 이라는 **눈에 보이는 파일 하나**를 중심에 두니까 설명이 통함.

이제 진짜 코드 칠 차례임.

#seam #설계 #피벗 #Docker #YAML
