...
[#2]({{< ref "/post/seam/2-피벗" >}})에서 `seam.yaml` 로 방향을 잡고, 바로 Rust로 짜기 시작했다.
...
**2시간 25분에 커밋 18개.** 론칭 계획에는 이 범위가 `M+0 ~ M+3`, 그러니까 3개월로 잡혀 있었다.

## 왜 Rust인가

단일 바이너리로 떨어지는 게 제일 컸다.

```bash
curl -fsSL install.seam.dev | sh
```

이 한 줄로 끝나야 하는데, Python이면 런타임이랑 의존성이 따라온다. Node면 더 심한다.
**"AI 환경 관리 도구를 쓰려고 환경을 관리해야 하는"** 상황이 되면 안 됐다.

게다가 데몬으로 계속 떠 있어야 하고, 프록시라서 모든 AI 요청이 여길 통과한다. 메모리 사용량이랑 지연이 그대로 체감된다.
...
`.env` 나 config 파일에 평문으로 두는 걸 처음부터 안 하기로 했다.
`keyring` 크레이트를 쓰면 macOS Keychain / Linux Secret Service에 그대로 들어간다.
...
설정 파일(`~/.seam/config.toml`)에는 키가 안 들어간다. 포트, 기본 모델 같은 것만 들어간다.
...
메모리를 처음엔 SQLite로 잡았다가 JSONL(append-only)로 바꿨다.
...
append-only라 쓰기가 단순하고, 사람이 `tail -f` 로 그냥 볼 수 있다.
[정담]({{< ref "/post/정담/4-저장소" >}}) 때랑 비슷한 결론인데, 데이터가 작고 동시 쓰기가 없으면 DB가 필요 없다.
...
`seam run` 이 `seam.yaml` 의 native 툴들을 서브프로세스로 띄운다. 그럼 **누가 살아 있는지** 알아야 한다.

```
~/.seam/run/<name>.pid
```

PID 파일 쓰고, 살아 있는지는 `libc::kill(pid, 0)` 으로 확인한다. 시그널 0은 실제로 안 보내고 **보낼 수 있는지만** 검사한다. macOS/Linux 공통으로 된다.

15:43 커밋이 재밌는데, `watch_with_restart` 가 크래시를 감지하고 **자기가 다시 띄우려고** 했었다.
그러면 재시작 책임이 두 군데(감시자 + 코디네이터)로 갈라져서 유령 프로세스가 생긴다. `Crashed` 상태로 끊고 나가게 바꿨다.

## 15:43~17:17 — CLI 서브커맨드
...
17:17 커밋이 자잘한 것만 모아놨는데 전부 같은 종류다.
...
**전부 "실패 경로를 안 본 것"이다.** 잘 돌아갈 때만 생각하고 짠 코드.
...
Docker 이미지 흉내인데 레이어는 없다. 그냥 압축 파일 하나다.

레지스트리는 GitHub Contents API를 쓴다. 레포가 없으면 만들어준다.

17:28 커밋이 중요한데, **HTTP 응답 상태를 아무도 안 보고 있었다.**
...
`reqwest` 는 4xx/5xx를 에러로 안 본다. 네트워크가 성공했으면 성공이다.
`.error_for_status()?` 를 안 붙이면 **404 응답을 파싱하려다 이상한 데서 터진다.** 레포 생성 응답도 마찬가지였다.
...
프록시가 바디를 읽다가 실패하면 그걸 삼키고 있었다. 502로 올려보내게 바꾼다.

## 느낀 점

**18개 중 7개가 `fix:` 였고, 전부 직전 `feat:` 을 고치는 거였다.**
...
빠르게 밀고 바로 되돌아와서 고치는 식이었다. 기능 하나 넣고 다시 읽어보면 실패 경로가 비어 있다.
**"돌아가게 만들기"랑 "안 터지게 만들기"가 다른 작업**이라는 게 커밋 로그에 그대로 남았다.

그리고 3개월 계획을 2시간 반에 끝낸 건 기분은 좋은데, 이게 마냥 좋은 일인지는 [마지막 글]({{< ref "/post/seam/8-마무리" >}})에서 다시 생각해본다.
