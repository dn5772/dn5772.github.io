...
[#1]({{< ref "/post/abide/1-앱-뼈대" >}})에서 성경 본문을 **로컬 에셋**으로 넣었다. 개역한글 하나만.
영어 사용자를 생각하면 버전이 여러 개여야 한다.

3월 23일 새벽에 이걸 붙였는데, **한 시간 동안 커밋 15개**가 나왔다. 대부분 버그 수정이다.
...
원래 성경 전체가 파일 하나였다. 버전이 셋이 되면 그게 셋이 된다.
...
창세기를 읽는데 요한계시록까지 메모리에 올릴 이유가 없다.
[#1]({{< ref "/post/abide/1-앱-뼈대" >}})에서 만든 로컬 파서가 권 단위 인덱싱을 하고 있어서 쪼개는 건 어렵지 않았다.
...
여기서부터가 진짜다. `BibleVersion` 이라는 값 하나가 생기니까 **계층을 전부 통과해야 했다.**
...
[#1]({{< ref "/post/abide/1-앱-뼈대" >}})에서 3레이어로 가른 대가다. **파라미터 하나가 레이어를 다 뚫고 내려가야 한다.**

커밋 두 개(`01:37`, `01:41`)가 연달아 "인자 안 넘김"을 고치는 건데, 컴파일은 되는데 기본값으로 동작해서 화면에서만 티가 났다.

솔직히 이 부분은 레이어를 나눈 게 손해였다. 근데 **책 이름 영문 필드**(`01:26`)처럼 도메인 엔티티만 고치면 되는 건 또 깔끔했다. 반반이다.
...
버전·글꼴 크기·본문 스타일을 바꾸는 바텀시트다.

`01:44` 커밋이 [#1]({{< ref "/post/abide/1-앱-뼈대" >}})에서 디자인 시스템을 먼저 깐 값어치다.
처음엔 색을 직접 박았다가 **토큰으로 바꾼다.** 다크 모드에서 안 맞았다.

## 그리고 버그가 줄줄이

여기서부터 나흘 동안 UI 버그만 잡았다.
...
**11개 중 7개가 설정 시트 하나에서 나왔다.**

패턴이 보다.

**1. 오버레이 겹친다.** 묵상 화면에서 구절 미리보기를 띄우면 **키보드가 올라올 때 입력창을 가린다.**
모바일에서 오버레이 + 키보드는 거의 항상 문제가 된다. 화면 높이가 실시간으로 바뀌는데 오버레이는 그걸 모른다.

**2. 터치 차단.** 투명한 오버레이가 **보이지도 않으면서 터치를 먹고 있었다.**
화면은 정상인데 버튼이 안 눌린다. 제일 찾기 어려운 종류다.

**3. 글자 크기가 반영이 안 된다.** 설정에서 키우면 값은 바뀌는데 **본문에 안 닿는다.**
프로바이더까지는 갔는데 위젯이 그 값을 구독하지 않고 있었다.

**4. 크기를 키우면 오버플로우.** 반영되게 고쳤더니 이번엔 시트가 터진다.
`01:44` 에서 고정 높이로 잡아둔 게 원인. 스크롤 가능하게 바꾼다.
...
무드 선택이나 빠른 진입 버튼에 이모지를 쓰고 있었다.

이모지는 **기기·OS 버전마다 그림이 다르다.** 같은 유니코드인데 안드로이드랑 iOS가 완전히 다르게 그림.
디자인을 맞출 수가 없고, 폰트가 없으면 두부(□)가 나온다.

실제로 `03-26 13:40` 에 "감사 무드 이모지 누락"이 있다. 아이콘으로 바꾼 뒤에도 한 군데가 남아 있었던 것.
...
**디버그 로그가 릴리즈 빌드에도 찍히고 있었다.** 토큰이나 응답 본문이 로그에 남는다.
[#4]({{< ref "/post/abide/4-인증" >}})에서 민감 정보 마스킹한 거랑 같은 줄기인데, 이번엔 클라이언트 쪽이었다.

## 느낀 점

**"버전 하나 추가"가 UI 버그 11개를 불렀다.**

기능 자체는 반나절이면 되는데, 그게 건드리는 화면이 성경 인덱스·본문·설정 시트·묵상 화면·홈 카드까지 퍼져 있었다.
**옵션이 하나 늘면 조합이 곱으로 늘어난다**는 걸 몸으로 배웠다.

그리고 모바일에서 **오버레이랑 키보드는 항상 의심**해야 한다. 11개 중 3개가 그거였다.

다음은 스트리밍이랑 Observer 깊이 판정이다. 이 시리즈에서 제일 오래 붙든 부분.
