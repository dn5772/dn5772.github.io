...
**ABIDE(말씀 안에 거하다)**. AI랑 같이 성경을 묵상하는 앱을 만들기 시작했다.

요약을 던져주는 게 아니라 **소크라테스식으로 되물으면서** 스스로 생각하게 만드는 게 목표다.
"이 구절이 무슨 뜻이냐"를 알려주는 앱은 이미 많은데, "당신한테 이게 왜 걸리냐"를 묻는 건 없었다.

첫날에 Flutter 앱 뼈대를 통째로 올렸다. 커밋 48개가 전부 `22:09~22:10` 에 찍혀 있다.

## 3레이어로 갈랐음

Clean Architecture 그대로 간다.
...
**의존 방향이 한쪽이다.** `presentation → domain ← data`.
`domain` 은 Flutter도 모르고 HTTP도 모른다. 순수 다트 객체랑 인터페이스만 있다.

이렇게 한 이유가 있다. 이 앱은 성경 본문을 **로컬 에셋에서** 읽고, 묵상 대화는 **서버 SSE에서** 받는다.
두 데이터소스의 성격이 완전히 다른데, 화면이 그걸 다 알고 있으면 나중에 못 바꾼다.
...
화면은 `BibleRepository` 만 알면 된다. 나중에 본문을 서버에서 받기로 해도 화면은 안 바뀐다.

## 디자인 시스템부터 깔았음

화면보다 테마를 먼저 만들었다. 커밋 순서가 그런다.
...
**Deep Forest + 글래스모피즘**으로 잡는다. 묵상 앱이라 차분해야 하는데, 그렇다고 밋밋하면 안 쓴다.
짙은 녹색 계열에 반투명 카드를 얹는 식으로 갔다.

색 → 타이포 → ThemeData → 공용 위젯 순서로 쌓은 게 나중에 편했다.
**화면마다 색을 직접 쓰는 일이 없어서** [#6]({{< ref "/post/abide/6-성경" >}})에서 설정 시트 같은 걸 추가할 때 토큰만 가져다 쓰면 됐다.
...
묵상 대화가 이 앱의 핵심인데, **한 번에 답이 다 나오면 안 된다.**
AI가 생각하는 동안 화면이 멈춰 있으면 대화가 아니라 로딩이다.

그래서 처음부터 Server-Sent Events로 잡았다. Dio 기반으로 직접 만들었고, 스트림 생명주기(구독·취소·에러·재연결)는 Riverpod 프로바이더가 들고 있게 한다.

이게 나중에 [#7]({{< ref "/post/abide/7-스트리밍" >}})에서 크게 확장된다.
...
한국어가 1차인데 영어 문자열 맵을 같이 깔아뒀다.
나중에 붙이려면 화면 14개를 다 뒤져야 하니까, 비어 있더라도 구조는 먼저 만들어두는 게 낫다.

…라고 생각했는데 **이게 절반만 맞았다.** 앱 UI는 이걸로 됐는데, **AI가 생성하는 텍스트**는 전혀 대비가 안 돼 있었다.
그 대가를 [#8]({{< ref "/post/abide/8-다국어" >}})에서 치른다.
...
묵상 중에 빨간 Flutter 에러 화면이 뜨면 그날 묵상은 끝이다. 이건 기능이 아니라 **분위기 문제**라서 처음부터 넣었다.

## 느낀 점

48커밋을 한 번에 올린 건데, **커밋을 잘게 쪼갠 게 나중에 읽기 좋았다.**
`feat(core/theme)`, `feat(domain)`, `feat(data)`, `feat(ui)` 로 레이어를 접두사에 박아두니까 나중에 "이때 뭘 깔았지"가 로그만 봐도 보다.

그리고 **화면보다 테마랑 라우터를 먼저 만든 순서**가 맞았다.
반대로 갔으면 화면 14개에 색이랑 네비게이션이 제각각 박혔을 것.

다음은 서버다. 앱은 있는데 부를 데가 없다.
