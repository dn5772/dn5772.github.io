...
메뉴가 바뀔 때마다 내가 JSON 고치고 배포하는 건 말이 안 됨.
**사장님이 직접 고칠 수 있어야 함.** 그래서 관리자 페이지를 만들었음.
...
쓰는 사람이 사장님 한 명임. 회원가입도 없고 권한 구분도 없음.
그래서 **유저 테이블도 안 만들었음.**
...
유저가 한 명일 때 DB를 안 만드는 건 게으른 게 아니라 맞는 선택인 것 같음. 테이블 하나 만들면 마이그레이션이 따라오고 백업이 따라옴.
...
전체 저장(`POST`)이랑 항목 저장(`PATCH`)을 **둘 다** 만들어둔 게 나중에 문제가 됨.
관리자 화면에 저장 버튼이 두 개 생기는데, 사장님 입장에선 뭘 눌러야 하는지 알 수가 없음. 이건 [#8]({{< ref "/post/정담/8-모바일" >}})에서 정리함.
...
받아서 저장하고 URL을 돌려줌. 메뉴 항목엔 그 URL만 들어감.

항목을 **삭제할 때 이미지도 같이 지우게** 해놨음. 안 그러면 안 쓰는 이미지가 계속 쌓임.
...
메뉴판은 순서가 중요함. 대표 메뉴가 맨 위에 있어야 함.

`@dnd-kit` 을 씀. react-beautiful-dnd가 더 유명한데 유지보수가 멈췄고, DnD Kit은 **터치를 기본으로 지원**함.
사장님이 폰으로 쓸 걸 생각하면 이게 결정적이었음.

## 🎯 느낀 점

하루에 1045줄은 좀 무리였음. 돌아가긴 하는데 **급하게 짠 흔적이 그대로 남음.**
디버깅한다고 넣은 `console.log`, "일단 되게" 하려고 건너뛴 검증, 저장 버튼 두 개 같은 것들.

두 달 뒤에 그걸 전부 되갚게 됨..

그리고 아직 제일 큰 문제가 안 드러났음. **이 메뉴 데이터를 어디에 저장할 거냐** 하는 것.
로컬에선 완벽하게 잘 돌아갔는데, 오늘 저녁에 배포하고 나서 알게 됨.

#정담 #관리자 #JWT #bcrypt #DndKit
